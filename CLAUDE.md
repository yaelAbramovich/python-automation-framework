...
5. **Use Playwright's `expect()` wherever it applies; plain `assert` (with a message) for
   everything else — and never a manual wait.** `expect()` accepts a `Page`, `Locator`, or
   `APIResponse` and auto-retries until the timeout (see `BaseApiClient._assert_response_is_ok`);
   never `assert locator.is_visible()` or `locator.wait_for()` as a pre-action gate — Playwright's
...
6. **`BasePage` stays generic; page-specific behavior belongs on the concrete page.** If a page
   ...
   class — never to `BasePage`.

7. **Small methods, then one that combines them when it helps.** Each POM method should do one
   thing — see `WikipediaTestAutomationPage`'s `click_test_driven_development_table_of_contents_link`.
   A composite method chains atomics together when a flow is reused across tests.

8. **Logger names are automatic.** Don't add a `logger_name` parameter back to `BasePage` or
   `BaseApiClient` — it's derived from the class name already.

9. **A test reaches a page object only through its fixture in `conftest.py`.** Never
   `SomePage(page)` directly inside a test — every POM gets a fixture named after its file
   (`wikipedia_test_automation_page.py` → fixture `wikipedia_test_automation_page`).

10. **mypy only checks `src/`, not `tests/`.** Code moved from a test file into `src/` may need
...
- **New page:** add its text to `strings.json` under `pages.<name>.*`. Create
  ...
  methods plus a combined one where it helps. Add a matching fixture to `conftest.py`. Also add
  whatever assertion methods make sense for that page (e.g. "form shows this error") — on the
  page class itself, never on `BasePage`.
